...
def _load_yfinance_ohlcv(
    ...
    if isinstance(frame.columns, pd.MultiIndex):
        ...
        for level in range(frame.columns.nlevels):
            ...
            if normalized_symbol in values:
                matched_level = level
                break
        if matched_level is None:
            ticker_like_levels = [
                level
                for level in range(frame.columns.nlevels)
                if len(set(frame.columns.get_level_values(level))) == 1
            ]
            if not ticker_like_levels:
                raise DataLoadError(f"missing yfinance ticker columns for {symbol}")
            matched_level = ticker_like_levels[-1]
            selected_key = next(iter(set(frame.columns.get_level_values(matched_level))))
        else:
            selected_key = next(
                value
                for value in frame.columns.get_level_values(matched_level)
                if str(value).upper() == normalized_symbol
            )
        frame = frame.xs(selected_key, axis=1, level=matched_level, drop_level=True)
        if isinstance(frame.columns, pd.MultiIndex):

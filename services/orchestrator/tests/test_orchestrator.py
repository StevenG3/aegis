...
def test_from_scorecard_market_buy_paper_executes(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    monkeypatch.setattr(orchestrator_app, "EV_GATE_MODE", "shadow")
    ...
    with orchestrator_app.connect() as conn:
        ev = conn.execute(
            "select scorecard_id, outcome_id, mode, gate_result, p, tp_pct, sl_pct, ev "
            "from ev_estimates where scorecard_id = ?",
            (scorecard_id,),
        ).fetchone()
    assert ev["scorecard_id"] == scorecard_id
    assert ev["outcome_id"] is not None
    assert ev["mode"] == "shadow"
    assert ev["gate_result"] == "pass"
    assert ev["p"] == "0.50000000"
    assert ev["tp_pct"] == "0.15789474"
    assert ev["sl_pct"] == "0.05263158"
    assert Decimal(ev["ev"]) > 0
...
def test_factor_attribution_aggregates_closed_outcomes(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    scorecard_id = "22222222-2222-4222-8222-222222222222"
    with orchestrator_app.connect() as conn:
        conn.execute(
            "insert into scorecards "
            "(scorecard_id, actor, symbol, action, source, payload_json, created_at, expires_at) "
    ...
    base = {
        "scorecard_id": scorecard_id,
        "actor": "tg_1",
        "closed_realized_pnl": "10.00000000",
    }
    orchestrator_app._update_factor_attribution(base)
    orchestrator_app._update_factor_attribution({**base, "closed_realized_pnl": "-4.00000000"})

    body = (
        TestClient(orchestrator_app.app).get("/factor-attribution", params={"actor": "tg_1"}).json()
    )
    by_factor = {(item["factor"], item["direction"]): item for item in body["items"]}
    assert by_factor[("market", "support")]["support_count"] == 2
    assert by_factor[("market", "support")]["win_count"] == 1
    assert by_factor[("market", "support")]["win_rate"] == "0.5000"
    assert by_factor[("market", "support")]["avg_pnl"] == "3.00000000"
    assert by_factor[("market", "support")]["loss_contribution"] == "4.00000000"
    assert by_factor[("news", "oppose")]["loss_contribution"] == "4.00000000"
...
def test_scheduler_tick_fires_due_watchlist_and_reschedules(monkeypatch, tmp_path: Path) -> None:
    ...
    with orchestrator_app.connect() as conn:
        ...
        conn.commit()
    fired: list[tuple[str, str, str, str, str | None]] = []
    ...
    def fake_fire(
        ...
        asset_type: str,
        *,
        source_origin: str = "standard",
        gate_conviction: str | None = None,
    ) -> bool:
        fired.append((actor, symbol, asset_type, source_origin, gate_conviction))
        return True
    ...
    assert fired == [("tg_1", "ETHUSDT", "crypto", "standard", None)]
    with orchestrator_app.connect() as conn:
        row = conn.execute(
...
def test_paper_feedback_bootstrap_default_off(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    monkeypatch.delenv("PAPER_FEEDBACK_BOOTSTRAP_ENABLED", raising=False)
    monkeypatch.setenv("PAPER_FEEDBACK_ACTOR", "tg_1")

    result = orchestrator_app.bootstrap_paper_feedback_loop(
        now=datetime(2026, 5, 25, 0, 0, tzinfo=UTC)
    )

    assert result["enabled"] is False
    with orchestrator_app.connect() as conn:
        assert conn.execute("select count(*) from watchlist_entries").fetchone()[0] == 0
        assert conn.execute("select count(*) from autonomy_settings").fetchone()[0] == 0


def test_paper_feedback_bootstrap_seeds_watchlist_and_paper_autonomy(
    monkeypatch, tmp_path: Path
) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    monkeypatch.setenv("PAPER_FEEDBACK_BOOTSTRAP_ENABLED", "true")
    monkeypatch.setenv("PAPER_FEEDBACK_ACTOR", "tg_1")
    monkeypatch.setenv("PAPER_FEEDBACK_SYMBOLS", "BTCUSDT, ETHUSDT:crypto, NVDA:stock")
    monkeypatch.setenv("PAPER_FEEDBACK_CADENCE_MINUTES", "120")
    monkeypatch.setenv("PAPER_FEEDBACK_DAILY_BUDGET_USDT", "30")
    monkeypatch.setenv("PAPER_FEEDBACK_PER_TRADE_USDT", "10")
    monkeypatch.setenv("PAPER_FEEDBACK_MIN_CONVICTION", "0.65")
    now = datetime(2026, 5, 25, 0, 0, tzinfo=UTC)

    result = orchestrator_app.bootstrap_paper_feedback_loop(now=now)

    assert result["enabled"] is True
    assert result["watchlist_upserted"] == 3
    with orchestrator_app.connect() as conn:
        rows = conn.execute(
            "select symbol, asset_type, cadence_minutes, next_run_at, enabled, "
            "source_origin, gate_conviction "
            "from watchlist_entries where actor = ? order by symbol",
            ("tg_1",),
        ).fetchall()
    ...
    assert all(row["cadence_minutes"] == 120 for row in rows)
    assert all(row["next_run_at"] == now.isoformat() for row in rows)
    assert all(row["enabled"] == 1 for row in rows)
    assert all(row["source_origin"] == "paper_feedback_bootstrap" for row in rows)
    assert all(row["gate_conviction"] == "0.65" for row in rows)
    assert settings["enabled"] == 1
    assert settings["daily_budget_usdt"] == "30"
    assert settings["per_trade_usdt"] == "10"
    assert settings["min_conviction"] == "0.65"
    assert settings["allowed_sources"] == "tradingagents"


def test_paper_feedback_bootstrap_off_disables_bootstrap_owned_rows(
    monkeypatch, tmp_path: Path
) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    monkeypatch.setenv("PAPER_FEEDBACK_BOOTSTRAP_ENABLED", "true")
    monkeypatch.setenv("PAPER_FEEDBACK_ACTOR", "tg_1")
    monkeypatch.setenv("PAPER_FEEDBACK_SYMBOLS", "BTCUSDT,ETHUSDT")
    monkeypatch.setenv("PAPER_FEEDBACK_DAILY_BUDGET_USDT", "30")
    monkeypatch.setenv("PAPER_FEEDBACK_PER_TRADE_USDT", "10")
    monkeypatch.setenv("PAPER_FEEDBACK_MIN_CONVICTION", "0.65")
    monkeypatch.setenv("PAPER_FEEDBACK_ALLOWED_SOURCES", "tradingagents")
    now = datetime(2026, 5, 25, 0, 0, tzinfo=UTC)
    orchestrator_app.bootstrap_paper_feedback_loop(now=now)

    monkeypatch.setenv("PAPER_FEEDBACK_BOOTSTRAP_ENABLED", "false")
    result = orchestrator_app.bootstrap_paper_feedback_loop(now=now + timedelta(minutes=1))

    assert result["enabled"] is False
    assert result["disabled_watchlist_entries"] == 2
    assert result["disabled_autonomy_settings"] == 1
    with orchestrator_app.connect() as conn:
        assert (
            conn.execute(
                "select count(*) from watchlist_entries "
                "where actor = ? and source_origin = ? and enabled = 0",
                ("tg_1", "paper_feedback_bootstrap"),
            ).fetchone()[0]
            == 2
        )
        settings = conn.execute(
...
def test_paper_bootstrap_guardrail_halts_on_drawdown_and_requires_resume(
    monkeypatch, tmp_path: Path
) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    monkeypatch.setenv("PAPER_FEEDBACK_BOOTSTRAP_ENABLED", "true")
    ...
    monkeypatch.setenv("PAPER_FEEDBACK_DAILY_BUDGET_USDT", "30")
    monkeypatch.setenv("PAPER_FEEDBACK_PER_TRADE_USDT", "10")
    monkeypatch.setattr(orchestrator_app, "AUTONOMY_DRAWDOWN_HALT_USDT", Decimal("25"))
    monkeypatch.setattr(orchestrator_app, "_notify_actor_event", lambda *args, **kwargs: True)
    now = datetime(2026, 5, 25, 0, 0, tzinfo=UTC)
    orchestrator_app.bootstrap_paper_feedback_loop(now=now)
    with orchestrator_app.connect() as conn:
        conn.execute(
            "insert into daily_pnl (actor, date, realized_delta, symbol, venue, created_at) "
            "values (?,?,?,?,?,?)",
            ("tg_1", "2026-05-25", "5", "BTCUSDT", "binance_spot", "a"),
        )
        conn.execute(
            "insert into daily_pnl (actor, date, realized_delta, symbol, venue, created_at) "
            "values (?,?,?,?,?,?)",
            ("tg_1", "2026-05-25", "-40", "BTCUSDT", "binance_spot", "b"),
        )
        conn.commit()

    halted = orchestrator_app.evaluate_paper_bootstrap_guardrails(actor="tg_1", now=now)

    assert halted["halted"] is True
    assert halted["new_halt"] is True
    assert halted["triggers"] == ["drawdown"]
    assert halted["alert_sent"] == 1
    with orchestrator_app.connect() as conn:
        watchlist = conn.execute(
            "select enabled from watchlist_entries where actor = ? and source_origin = ?",
            ("tg_1", "paper_feedback_bootstrap"),
        ).fetchone()
        settings = conn.execute(
            "select enabled from autonomy_settings where actor = ?", ("tg_1",)
        ).fetchone()
    assert watchlist["enabled"] == 0
    assert settings["enabled"] == 0

    blocked = orchestrator_app.bootstrap_paper_feedback_loop(now=now + timedelta(minutes=1))
    ...
    resumed = orchestrator_app._resume_paper_feedback_bootstrap(
        "tg_1", resumed_by="test", now=now + timedelta(minutes=2)
    )
    assert resumed["halted"] is False
    assert resumed["restored"]["watchlist_upserted"] == 1
    with orchestrator_app.connect() as conn:
        assert (
            conn.execute(
                "select enabled from watchlist_entries where actor = ? and source_origin = ?",
...
def test_paper_bootstrap_guardrail_halts_on_mark_failure_streak(
    monkeypatch, tmp_path: Path
) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    monkeypatch.setenv("PAPER_FEEDBACK_BOOTSTRAP_ENABLED", "true")
    monkeypatch.setenv("PAPER_FEEDBACK_ACTOR", "tg_1")
    monkeypatch.setenv("PAPER_FEEDBACK_SYMBOLS", "ETHUSDT")
    monkeypatch.setattr(orchestrator_app, "AUTONOMY_MARK_FAILURE_HALT_N", 2)
    monkeypatch.setattr(orchestrator_app, "_notify_actor_event", lambda *args, **kwargs: False)
    monkeypatch.setattr(orchestrator_app, "_mark_for_symbol", lambda symbol: (None, None))
    now = datetime(2026, 5, 25, 0, 0, tzinfo=UTC)
    orchestrator_app.bootstrap_paper_feedback_loop(now=now)
    with orchestrator_app.connect() as conn:
        conn.execute(
    ...
    first = orchestrator_app.evaluate_paper_bootstrap_guardrails(actor="tg_1", now=now)
    second = orchestrator_app.evaluate_paper_bootstrap_guardrails(
        actor="tg_1", now=now + timedelta(minutes=1)
    )

    assert first["halted"] is False
    assert first["observed"]["mark_failure_streak"] == 1
...
def test_paper_bootstrap_guardrail_halts_on_decision_error_rate(
    ...
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    monkeypatch.setenv("ANALYSIS_ADAPTER_DB_PATH", str(tmp_path / "analysis_adapter.sqlite"))
    monkeypatch.setenv("PAPER_FEEDBACK_BOOTSTRAP_ENABLED", "true")
    monkeypatch.setenv("PAPER_FEEDBACK_ACTOR", "tg_1")
    monkeypatch.setenv("PAPER_FEEDBACK_SYMBOLS", "BTCUSDT")
    ...
    adapter_conn.execute("create table analysis_jobs (actor text, status text, requested_at text)")
    for status in ("failed", "failed", "succeeded"):
        adapter_conn.execute(
            "insert into analysis_jobs (actor, status, requested_at) values (?,?,?)",
            ("tg_1", status, now.isoformat()),
        )
    adapter_conn.commit()
    adapter_conn.close()
    ...
    assert observed["failure_rate"] == "0.66666667"


def test_paper_bootstrap_status_exposes_progress_and_human_gate(
    monkeypatch, tmp_path: Path
) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    monkeypatch.setenv("PAPER_FEEDBACK_BOOTSTRAP_ENABLED", "true")
    monkeypatch.setenv("PAPER_FEEDBACK_ACTOR", "tg_1")
    monkeypatch.setenv("PAPER_FEEDBACK_SYMBOLS", "BTCUSDT")
    orchestrator_app.bootstrap_paper_feedback_loop(now=datetime(2026, 5, 25, 0, 0, tzinfo=UTC))
    client = TestClient(orchestrator_app.app)

    status = client.get("/paper/bootstrap/status", params={"actor": "tg_1"}).json()

    assert status["bootstrap"]["env_enabled"] is True
    assert status["bootstrap"]["watchlist_enabled_count"] == 1
    assert status["bootstrap"]["guardrail_thresholds"]["drawdown_halt_usdt"] == "25.00000000"
    progress = status["feedback_loop_progress"]
    assert progress["ev_shadow"]["ready_n"] == 20
    assert progress["ready_for_feedback_loop_checkup"] is False
...
def _seed_calibration_outcome(
    *,
    ...
    reflected_at: str | None = "2026-05-26T00:01:00+00:00",
    ...
    if gate_conviction is not None:
        metadata["gate_conviction"] = gate_conviction
    with orchestrator_app.connect() as conn:
        conn.execute(
            "insert into scorecards "
            "(scorecard_id, actor, symbol, action, source, payload_json, created_at, expires_at) "
...
def test_recompute_bucketing_correct(monkeypatch, tmp_path: Path) -> None:
    ...
    client = TestClient(orchestrator_app.app)
    recompute = client.post("/calibration/recompute").json()
    assert recompute["buckets_written"] == 2
    assert recompute["data_origin_breakdown"]["tradingagents:crypto:0.60-0.70"] == {
        "standard": {"n": 8, "gates": {"standard": 8}}
    }
    ...
    assert by_bucket["0.60-0.70"]["data_origin_breakdown"] == {
        "standard": {"n": 8, "gates": {"standard": 8}}
    }


def test_calibration_report_distinguishes_bootstrap_origin(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    _seed_calibration_outcome(
        heuristic="0.6500",
        origin="paper_feedback_bootstrap",
        gate_conviction="0.65",
    )
    _seed_calibration_outcome(heuristic="0.6500")
    client = TestClient(orchestrator_app.app)

    assert client.post("/calibration/recompute").json()["buckets_written"] == 1
    item = client.get("/calibration", params={"source": "tradingagents"}).json()["items"][0]

    assert item["heuristic_bucket"] == "0.60-0.70"
    assert item["sample_count"] == 2
    assert item["data_origin_breakdown"] == {
        "paper_feedback_bootstrap": {"n": 1, "gates": {"0.65": 1}},
        "standard": {"n": 1, "gates": {"standard": 1}},
    }


def test_recompute_includes_closed_outcomes_pending_external_reflection(
    monkeypatch, tmp_path: Path
) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    _seed_calibration_outcome(reflected_at=None)
    client = TestClient(orchestrator_app.app)

    response = client.post("/calibration/recompute").json()
    assert response["buckets_written"] == 1
    assert response["rows_considered"] == 1
    assert response["data_origin_breakdown"] == {
        "tradingagents:crypto:0.70-0.80": {"standard": {"n": 1, "gates": {"standard": 1}}}
    }
    items = client.get("/calibration", params={"source": "tradingagents"}).json()["items"]
    assert items[0]["sample_count"] == 1
    assert items[0]["hit_count"] == 1
...
def _seed_calibration(source: str = "tradingagents", asset_type: str = "crypto") -> None:
    with orchestrator_app.connect() as conn:
        conn.execute(
            "insert into conviction_calibration values (?,?,?,?,?,?,?,?,?)",
            (source, asset_type, "0.70-0.80", 20, 12, "0", "0.6", "0.6", "now"),
        )
        conn.commit()
...
def _live_settings() -> dict[str, object]:
    return {
        ...
        "daily_live_trade_count_max": 3,
        "min_calibrated_conviction": "0.50",
...
def test_ev_gate_shadow_records_negative_ev_without_blocking(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    monkeypatch.setattr(orchestrator_app, "LIVE_AUTONOMY_GLOBAL_ENABLED", True)
    monkeypatch.setattr(orchestrator_app, "EV_GATE_MODE", "shadow")
    _seed_calibration()
    payload = {
        "scorecard_id": "sc-ev-shadow",
        "source": "tradingagents",
        "symbol": "BTCUSDT",
        "action": "buy",
        "entry_low": "100",
    ...
    assert orchestrator_app._eligible_for_live_auto("tg_1", payload, _live_settings()) == (
        True,
        "OK",
    )
    with orchestrator_app.connect() as conn:
        row = conn.execute("select mode, gate_result, ev from ev_estimates").fetchone()
    assert row["mode"] == "shadow"
    assert row["gate_result"] == "pass"
    assert Decimal(row["ev"]) < 0


def test_ev_gate_enforce_blocks_below_min_ev(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    monkeypatch.setattr(orchestrator_app, "LIVE_AUTONOMY_GLOBAL_ENABLED", True)
    monkeypatch.setattr(orchestrator_app, "EV_GATE_MODE", "enforce")
    monkeypatch.setattr(orchestrator_app, "MIN_EV", Decimal("0"))
    _seed_calibration()
    payload = {
        "scorecard_id": "sc-ev-enforce",
        "source": "tradingagents",
        "symbol": "BTCUSDT",
        "action": "buy",
        "entry_low": "100",
...
def test_ev_gate_enforce_allows_positive_ev(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    monkeypatch.setattr(orchestrator_app, "LIVE_AUTONOMY_GLOBAL_ENABLED", True)
    monkeypatch.setattr(orchestrator_app, "EV_GATE_MODE", "enforce")
    monkeypatch.setattr(orchestrator_app, "MIN_EV", Decimal("0"))
    ...
    estimate = orchestrator_app._estimate_ev(payload, Decimal("0.6"))
    assert estimate["ev"] == "0.03840000"


def test_ev_gate_missing_inputs_records_without_blocking(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    monkeypatch.setattr(orchestrator_app, "LIVE_AUTONOMY_GLOBAL_ENABLED", True)
    monkeypatch.setattr(orchestrator_app, "EV_GATE_MODE", "enforce")
    _seed_calibration()
    payload = {
        "scorecard_id": "sc-ev-missing",
        "source": "tradingagents",
        "symbol": "BTCUSDT",
        "action": "buy",
        "entry_low": "100",
    ...
    with orchestrator_app.connect() as conn:
        row = conn.execute("select reason, ev from ev_estimates").fetchone()
    assert row["reason"] == "EV_INPUTS_MISSING"
    assert row["ev"] is None


def _seed_ev_shadow_sample(
    scorecard_id: str,
    outcome_id: str,
    *,
    ev: str,
    ...
    metadata: dict[str, str] = {"asset_type": "crypto", "heuristic_conviction": "0.7000"}
    if origin is not None:
        metadata["origin"] = origin
    if gate_conviction is not None:
        metadata["gate_conviction"] = gate_conviction
    with orchestrator_app.connect() as conn:
        conn.execute(
            ...
            (
                ...
                "buy",
                "tradingagents",
                json.dumps({"conviction": "0.7000", "metadata": metadata}),
                "2026-05-25T00:00:00+00:00",
                "2026-05-26T00:00:00+00:00",
            ),
        )
...
def test_ev_shadow_report_flags_insufficient_samples(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    _seed_ev_shadow_sample("sc-low-n", "out-low-n", ev="-0.01000000", pnl="-5")
    report = orchestrator_app.build_ev_shadow_report(actor="tg_1")
    assert list(report)[0] == "sample"
    assert report["sample"]["n"] == 1
    assert report["sample"]["data_origin_breakdown"] == {
        "standard": {"n": 1, "gate_convictions": {"standard": 1}}
    }
    assert report["recommendation"]["recommend_enforce"] is False
    assert "数据不足" in report["recommendation"]["message"]


def test_ev_shadow_report_distinguishes_bootstrap_origin(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    _seed_ev_shadow_sample("sc-standard", "out-standard", ev="0.01000000", pnl="1")
    _seed_ev_shadow_sample(
        "sc-bootstrap",
        "out-bootstrap",
        ev="0.02000000",
        pnl="-1",
    ...
    assert report["sample"]["data_origin_breakdown"] == {
        "paper_feedback_bootstrap": {"n": 1, "gate_convictions": {"0.65": 1}},
        "standard": {"n": 1, "gate_convictions": {"standard": 1}},
    }


def test_ev_shadow_report_counterfactual_and_writes_files(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path / "data"))
    monkeypatch.setattr(orchestrator_app, "EV_SHADOW_MIN_SAMPLES", 4)
    monkeypatch.setattr(orchestrator_app, "EV_SHADOW_REPORT_DIR", str(tmp_path / "reports"))
    _seed_ev_shadow_sample("sc-pos-win-1", "out-pos-win-1", ev="0.04000000", pnl="8")
    _seed_ev_shadow_sample("sc-pos-win-2", "out-pos-win-2", ev="0.02000000", pnl="3")
    _seed_ev_shadow_sample("sc-neg-loss", "out-neg-loss", ev="-0.03000000", pnl="-5")
    _seed_ev_shadow_sample("sc-neg-win", "out-neg-win", ev="-0.01000000", pnl="2")
    client = TestClient(orchestrator_app.app)

    response = client.get("/ev-shadow/report", params={"actor": "tg_1", "write_report": True})

    assert response.status_code == 200
    body = response.json()
    assert body["sample"]["n"] == 4
    assert body["groups"]["ev_gt_0"]["win_rate"] == "1.00000000"
    assert body["groups"]["ev_lt_0"]["avg_realized_pnl"] == "-1.50000000"
    assert body["counterfactual_enforce"]["avoided_loss_count"] == 1
    assert body["counterfactual_enforce"]["false_kill_profit_count"] == 1
    assert body["counterfactual_enforce"]["net_impact_usdt"] == "3.00000000"
    assert body["recommendation"]["recommend_enforce"] is True
    assert Path(body["written_files"]["json"]).exists()
    assert Path(body["written_files"]["text"]).exists()
...
def test_live_exposure_cap_defaults_closed_and_blocks_breach(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    monkeypatch.setattr(orchestrator_app, "LIVE_AUTONOMY_GLOBAL_ENABLED", True)
    settings = {
        "enabled": True,
...
def test_live_autonomy_settings_include_us_equity_exposure_cap(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    client = TestClient(orchestrator_app.app)
    assert (
        client.post(
...
def test_live_auto_stock_requires_us_equity_exposure_cap(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    monkeypatch.setattr(orchestrator_app, "LIVE_AUTONOMY_GLOBAL_ENABLED", True)
    settings = {
        "enabled": True,
...
def test_paper_autonomy_observability_and_alert_notification(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    monkeypatch.setattr(orchestrator_app, "AUTONOMY_DRAWDOWN_ALERT_USDT", Decimal("100"))
    monkeypatch.setattr(orchestrator_app, "AUTONOMY_CONSECUTIVE_LOSS_ALERT_N", 2)
    monkeypatch.setattr(
        orchestrator_app, "NOTIFICATION_HOST_ALLOWLIST", frozenset({"hermes-agent"})
    )
    monkeypatch.setattr(
        orchestrator_app,
        "_mark_for_symbol",
        lambda symbol: (None, None) if symbol == "ETHUSDT" else (Decimal("110"), "test"),
    )
    with orchestrator_app.connect() as conn:
        for symbol, avg_cost, realized in (("BTCUSDT", "100", "7"), ("ETHUSDT", "50", "0")):
            conn.execute(
                """
                insert into paper_positions
                  (actor, symbol, qty, avg_cost, total_cost, realized_pnl,
        ...
        conn.execute(
            "insert into daily_pnl (actor, date, realized_delta, symbol, venue, created_at) "
            "values (?,?,?,?,?,?)",
            ("tg_1", orchestrator_app._today(), "10", "BTCUSDT", "binance_spot", "a"),
        )
        conn.execute(
            "insert into daily_pnl (actor, date, realized_delta, symbol, venue, created_at) "
            "values (?,?,?,?,?,?)",
            ("tg_1", orchestrator_app._today(), "-150", "BTCUSDT", "binance_spot", "b"),
        )
        conn.execute(
            "insert into intents "
            "(intent_id, payload_json, decision_json, execution_json, created_at, status, "
            "idempotency_key) values (?,?,?,?,?,?,?)",
            (
                "intent-paper-observe",
                json.dumps({**VALID, "actor": "tg_1", "mode": "paper"}),
                json.dumps(decision()),
                json.dumps(execution()),
                f"{orchestrator_app._today()}T00:00:00+00:00",
                "executed",
                "paper-observe",
            ),
        )
        conn.commit()
    _seed_ev_shadow_sample("loss-a", "loss-out-a", ev="-0.1", pnl="-1")
    _seed_ev_shadow_sample("loss-b", "loss-out-b", ev="-0.2", pnl="-2")
    client = TestClient(orchestrator_app.app)
    overview = client.get("/paper/autonomy/observability", params={"actor": "tg_1"}).json()
    assert overview["today_decision_count"] == 1
    assert overview["current_unrealized_pnl"] == "10.00000000"
    assert overview["max_drawdown_today_usdt"] == "150.00000000"
    assert {alert["type"] for alert in overview["alerts"]} == {
        "drawdown",
        "consecutive_losses",
    ...
    subscribed = client.post(
        ...
        json={
            "actor": "tg_1",
            "webhook_url": "http://hermes-agent:9090/trading/alert",
            "events": ["alert"],
        },
    )
    assert subscribed.status_code == 200
    ...
    def fake_post(url: str, **kwargs: object) -> FakeResponse:
        if url == "http://hermes-agent:9090/trading/alert":
            calls.append(kwargs)
            return FakeResponse({"ok": True}, status_code=204)
        return FakeResponse({"ok": True})

    monkeypatch.setattr(orchestrator_app.httpx, "post", fake_post)
    alerted = client.post("/paper/autonomy/alerts", params={"actor": "tg_1", "notify": True}).json()
    assert alerted["sent"] == 3
    assert len(calls) == 3
...
def test_stop_loss_watchdog_fires_paper_protective_sell(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
...
def test_stop_loss_watchdog_mints_bound_token_for_live_sell(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    monkeypatch.setattr(orchestrator_app, "STOP_LOSS_WATCHDOG_ENABLED", True)
    monkeypatch.setattr(
        orchestrator_app,
...
def test_memory_recall_aggregates_existing_domain_tables(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    client = TestClient(orchestrator_app.app)
    scorecard_id = client.post(
        "/scorecards",
    ...
    outcome_id = "11111111-1111-4111-8111-111111111112"
    with orchestrator_app.connect() as conn:
        conn.execute(
            """
            insert into scorecard_outcomes
    ...
    response = client.get("/memory/recall", params={"subject": "BTCUSDT", "limit": 20})
    assert response.status_code == 200
    items = response.json()["items"]
    ids = {item["id"] for item in items}
    assert f"decision:scorecard:{scorecard_id}" in ids
    assert f"outcome:scorecard_outcome:{outcome_id}" in ids
    assert f"lesson:scorecard_outcome:{outcome_id}" in ids
    assert f"observation:ev_estimate:{scorecard_id}" in ids
    assert all(item["source_ref"]["table"] for item in items)

    lesson = client.get("/memory/recall", params={"subject": "BTCUSDT", "type": "lesson"}).json()[
        "items"
    ][0]
    assert lesson["source_ref"]["outcome_id"] == outcome_id
    assert "return_pct=0.05000000" in lesson["content"]

    calibration = client.get("/memory/recall", params={"q": "calibrated_conviction"}).json()[
        "items"
    ]
    assert any(item["source_ref"]["table"] == "conviction_calibration" for item in calibration)


def test_memory_recall_marks_superseded_scorecard_decisions(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    client = TestClient(orchestrator_app.app)
    older = client.post(
        "/scorecards",
        json=make_scorecard_payload(actor="tg_1", symbol="ETHUSDT", source="manual"),
    ).json()["scorecard_id"]
    newer = client.post(
        "/scorecards",
        json=make_scorecard_payload(actor="tg_1", symbol="ETHUSDT", source="manual"),
    ...
    default_items = client.get(
        "/memory/recall", params={"subject": "ETHUSDT", "type": "decision"}
    ).json()["items"]
    assert [item["id"] for item in default_items] == [f"decision:scorecard:{newer}"]

    history = client.get(
        "/memory/recall",
        params={"subject": "ETHUSDT", "type": "decision", "include_superseded": True},
    ).json()["items"]
    by_id = {item["id"]: item for item in history}
    assert by_id[f"decision:scorecard:{older}"]["superseded_by"] == f"decision:scorecard:{newer}"


def test_memory_lineage_returns_original_source_chain(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    client = TestClient(orchestrator_app.app)
    scorecard_id = client.post(
        "/scorecards",
        json=make_scorecard_payload(actor="tg_1", symbol="SOLUSDT", source="manual"),
    ).json()["scorecard_id"]

    response = client.get(f"/memory/lineage/decision:scorecard:{scorecard_id}")
    assert response.status_code == 200
    lineage = response.json()["lineage"]
    assert lineage[0]["kind"] == "memory_entry"
    assert any(item["kind"] == "scorecard_payload" for item in lineage)
    assert lineage[0]["data"]["source_ref"]["scorecard_id"] == scorecard_id


def test_memory_record_persists_and_recalls_written_memory(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    client = TestClient(orchestrator_app.app)
    scorecard_id = client.post(
        "/scorecards",
        json=make_scorecard_payload(actor="tg_1", symbol="BTCUSDT", source="manual"),
    ).json()["scorecard_id"]
    ...
    response = client.post(
        ...
        json={
            "type": "convention",
            "subject": "BTCUSDT",
            "content": "Prefer paper-only checks before any live BTCUSDT action.",
            "source_ref": {
                "table": "scorecards",
                "scorecard_id": scorecard_id,
    ...
    duplicate = client.post(
        ...
        json={
            "type": "convention",
            "subject": "BTCUSDT",
            "content": "Duplicate content should not create a second source record.",
            "source_ref": {
                "table": "scorecards",
                "scorecard_id": scorecard_id,
                "trigger": "manual_record",
    ...
    assert duplicate.json()["memory_id"] == memory_id

    recalled = client.get(
        "/memory/recall", params={"subject": "BTCUSDT", "type": "convention"}
    ).json()["items"]
    assert [item["id"] for item in recalled] == [memory_id]

    lineage = client.get(f"/memory/lineage/{memory_id}").json()["lineage"]
    assert lineage[0]["kind"] == "memory_entry"
    assert any(item["kind"] == "scorecard_payload" for item in lineage)


def test_risk_reject_records_observation_memory(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    client = TestClient(orchestrator_app.app)
    ...
    monkeypatch.setattr(orchestrator_app.httpx, "post", fake_post)
    response = client.post("/intents", json=dict(VALID, idempotency_key="risk-memory"))

    assert response.status_code == 422
    memories = client.get(
        "/memory/recall", params={"type": "observation", "q": "NOTIONAL_EXCEEDS_HARD_CAP"}
    ).json()["items"]
    assert len(memories) == 1
    assert memories[0]["source_ref"]["table"] == "intents"
    assert memories[0]["source_ref"]["trigger"] == "risk_reject"

    lineage = client.get(f"/memory/lineage/{memories[0]['id']}").json()["lineage"]
    assert any(item["kind"] == "intent" for item in lineage)


def test_closed_outcome_records_threshold_lesson_memory(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    client = TestClient(orchestrator_app.app)
    _risk_and_execution(monkeypatch, avg_price="100000.00", filled_qty="0.001")
    _create_scorecard_order(client, idempotency_key="memory-close-buy")
    _manual_sell(client, idempotency_key="memory-close-sell", qty="0.001", price="110000.00")

    memories = client.get(
        "/memory/recall", params={"type": "lesson", "q": "closed_outcome"}
    ).json()["items"]

    assert len(memories) == 1
    assert memories[0]["source_ref"]["table"] == "scorecard_outcomes"
    assert memories[0]["source_ref"]["trigger"] == "closed_outcome"
    assert "realized_pnl=10.00000000" in memories[0]["content"]
    assert "factors=" in memories[0]["content"]


def test_closed_outcome_records_relative_return_lesson_memory(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    monkeypatch.setenv("MEMORY_PNL_THRESHOLD", "10")
    monkeypatch.setenv("MEMORY_RETURN_THRESHOLD", "0.02")
    client = TestClient(orchestrator_app.app)
    _risk_and_execution(monkeypatch, avg_price="100000.00", filled_qty="0.001")
    _create_scorecard_order(client, idempotency_key="memory-return-buy")
    _manual_sell(client, idempotency_key="memory-return-sell", qty="0.001", price="103000.00")
    ...
    assert len(memories) == 1
    assert "realized_pnl=3.00000000" in memories[0]["content"]
    assert "return_pct=0.03000000" in memories[0]["content"]
    assert "significant_return" in memories[0]["tags"]


def test_reflection_records_lesson_memory(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    client = TestClient(orchestrator_app.app)
    scorecard_id = client.post(
        "/scorecards",
        json=make_scorecard_payload(actor="tg_1", symbol="ETHUSDT", source="tradingagents"),
    ).json()["scorecard_id"]
    outcome_id = "11111111-1111-4111-8111-111111111113"
    with orchestrator_app.connect() as conn:
        conn.execute(
            """
            insert into scorecard_outcomes
    ...
    monkeypatch.setattr(
        orchestrator_app.httpx,
        "post",
        lambda *args, **kwargs: FakeResponse({"ok": True, "reflected": True}),
    )
    response = client.post("/reflect/pending")

    assert response.status_code == 200
    assert response.json()["reflected"] == 1
    memories = client.get("/memory/recall", params={"type": "lesson", "q": "reflection"}).json()[
        "items"
    ]
    assert len(memories) == 1
    assert memories[0]["source_ref"]["trigger"] == "reflection"
    ...
    *,
    ...
    with orchestrator_app.connect() as conn:
        ...
        conn.execute(
            ...
            """,
            (
                TRAILING_OUTCOME_ID if mode == "paper" else "11111111-1111-4111-8111-000000000002",
                scorecard_id,
                actor,
                "BTCUSDT",
                "tradingagents",
...
def test_reconcile_clean_match_status_ok(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        ...
        "get",
        lambda url, **kw: (
            FakeBridgePositionsResponse(bridge_pos) if "positions" in url else FakePriceResponse()
        ),
    )
...
def test_reconcile_clean_match_still_works_with_ready_true(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    bridge_pos = [{"symbol": "NVDA", "qty": "10.00000000", "avg_cost": "450.25000000"}]
    monkeypatch.setattr(
        orchestrator_app.httpx,
        "get",
        lambda url, **kw: (
            FakeBridgePositionsResponse(bridge_pos, ready=True)
            if "positions" in url
...
def test_reconcile_error_when_bridge_not_ready(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        ...
        "get",
        lambda url, **kw: (
            FakeBridgePositionsResponse([], ready=False)
            if "positions" in url
            else FakePriceResponse()
        ),
    )
...
def test_reconcile_records_bridge_last_update(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        ...
        "get",
        lambda url, **kw: (
            FakeBridgePositionsResponse(bridge_pos, ready=True, last_update=last_update)
            if "positions" in url
            else FakePriceResponse()
        ),
    )
...
def test_post_reconcile_ibkr_returns_200_on_clean(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        ...
        "get",
        lambda url, **kw: (
            FakeBridgePositionsResponse([]) if "positions" in url else FakePriceResponse()
        ),
    )
    import sqlite3 as _sqlite3
...
def test_startup_hook_skips_when_stub_mode(monkeypatch, tmp_path: Path) -> None:
    ...
    called = []
    monkeypatch.setattr(orchestrator_app, "run_ibkr_reconciliation", lambda **kw: called.append(1))
    import asyncio

    asyncio.get_event_loop().run_until_complete(orchestrator_app._startup_ibkr_reconcile())
    assert called == []

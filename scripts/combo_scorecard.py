...
from aegis.combo_scorecard import ScorecardConfig, report_to_dict, run_combo_scorecard
from aegis.private_paths import private_dir_from_cli


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the read-only Olympus #46 predeclared combo scorecard study."
    )
    ...
    parser.add_argument("--profit-factor-threshold", type=float, default=1.30)
...
def main() -> int:
    ...
    report = run_combo_scorecard(bars_by_symbol, config=config, cost_model=cost_model)
    payload = {
    ...
    output_dir = private_dir_from_cli(args.private_dir, default_task="olympus46")
    output_dir.mkdir(parents=True, exist_ok=True)
    output_path = output_dir / f"combo-scorecard-{dt.datetime.now(dt.UTC):%Y%m%dT%H%M%SZ}.json"

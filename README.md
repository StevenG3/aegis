...
> Aegis is the public, generic trading-core repository for Olympus. It is not the official TradingAgents project and does not contain private account data,
> credentials, private strategy implementations, or private strategy validation outputs.

> Public repository boundary: real balances, positions, account identifiers,
> broker/exchange/API keys, private strategy code, private strategy evidence,
> and host-specific operational data must stay in private or ignored storage.

> Olympus positions Aegis as an honest hypothesis falsifier and risk-discipline tool:
> research candidates are kept paper-only until predeclared gates survive costs,
> benchmarks, and out-of-sample checks. A documented risk reference is not a
...
only when it can connect to Gateway/TWS; when disconnected it attempts a
cooldown-limited reconnect before returning `503`. Phase 22 unlocks live US
equities only behind separate gates: global `LIVE_TRADING_ENABLED=true`,
...
EV shadow replay and paper-autonomy observability are read-only. Reports use linked
`ev_estimates` + closed `scorecard_outcomes`, write aggregate JSON/text under the gitignored
private `../aegis-strategies/incubating/` evidence directory by default, and never
switch `EV_GATE_MODE` automatically. Set `OLYMPUS_EVIDENCE_DIR` or the script-specific
output environment variable when a different private evidence location is required.

```bash
curl -s 'http://127.0.0.1:8080/ev-shadow/report?actor=user_1'
python scripts/ev_shadow_report.py --actor user_1 --text
curl -s 'http://127.0.0.1:8080/paper/autonomy/observability?actor=user_1'
curl -s -X POST 'http://127.0.0.1:8080/paper/autonomy/alerts?actor=user_1&notify=true'
curl -s 'http://127.0.0.1:8080/paper/bootstrap/status?actor=user_1'
curl -s -X POST 'http://127.0.0.1:8080/paper/bootstrap/evaluate-guardrails?actor=user_1'
curl -s -X POST 'http://127.0.0.1:8080/paper/bootstrap/resume?actor=user_1&resumed_by=operator'
```
...
To avoid feedback-loop data starvation, operators can optionally bootstrap the existing paper pipeline from environment. `PAPER_FEEDBACK_BOOTSTRAP_ENABLED=false` by default. When explicitly set to `true`, the orchestrator requires `PAPER_FEEDBACK_ACTOR`, upserts `PAPER_FEEDBACK_SYMBOLS` into the watchlist, and enables paper autonomy for that actor using `PAPER_FEEDBACK_DAILY_BUDGET_USDT`, `PAPER_FEEDBACK_PER_TRADE_USDT`, `PAPER_FEEDBACK_MIN_CONVICTION`, and `PAPER_FEEDBACK_ALLOWED_SOURCES`. `PAPER_FEEDBACK_MIN_CONVICTION` defaults to `0.65`, below the standard `0.70` signal gate, so bootstrap-generated watchlist entries and scorecards are tagged with `origin=paper_feedback_bootstrap` and `gate_conviction=0.65`. The lower paper-only bootstrap gate is intended to produce outcomes for the `0.60-0.70` calibration bucket; EV shadow and calibration reports include data-origin breakdowns so bootstrap samples are not interpreted as standard-gate samples. This only seeds scheduled analysis and paper execution settings; EV, risk, notional, confirmation, and live-trading gates are unchanged.

Phase 17 adds paper bootstrap halt guardrails. On each scheduler/auto-trade tick, the orchestrator checks daily drawdown, consecutive closed losses, sustained mark-price failures, and recent analysis/decision-chain failure rate. If a halt threshold is breached, it disables bootstrap-owned watchlist rows and matching paper autonomy for that actor, sends an `alert` notification when subscribed, and requires an explicit `/paper/bootstrap/resume` call to restore. Defaults are conservative and only stop new paper entries: `AUTONOMY_DRAWDOWN_HALT_USDT=25`, `AUTONOMY_CONSECUTIVE_LOSS_HALT_N=3`, `AUTONOMY_MARK_FAILURE_HALT_N=3`, `AUTONOMY_DECISION_ERROR_RATE_HALT_PCT=0.50`, `AUTONOMY_DECISION_ERROR_RATE_MIN_N=3`, and `AUTONOMY_DECISION_ERROR_LOOKBACK_MINUTES=120`. Existing open paper positions are not force-closed by this guardrail; existing protective/risk logic remains separate.

`/paper/bootstrap/status` is the read-only progress check for Hermes or dashboards. It reports bootstrap enabled/paused state, today decision count, realized/unrealized PnL, drawdown, loss streak, EV sample count, closed-outcome count, automatic memory count, calibration bucket coverage, and whether the feedback loop has enough data for a human checkup. Default checkup thresholds are `FEEDBACK_EV_READY_N=20`, `FEEDBACK_CLOSED_OUTCOME_READY_N=20`, `FEEDBACK_CALIBRATION_READY_BUCKETS=2`, and `FEEDBACK_MEMORY_AUTOMATIC_READY_N=5`. Hitting those thresholds only recommends a human feedback-loop review; it never flips `EV_GATE_MODE` or changes decisions automatically.

Closed-outcome memory writes are also post-trade only. A lesson is recorded when realized PnL exceeds `MEMORY_PNL_THRESHOLD`, return exceeds `MEMORY_RETURN_THRESHOLD`, or EV direction mismatched the result. These memory entries are audit artifacts and are not read back into automated trade decisions.
...
```bash
curl -s -X POST http://127.0.0.1:8080/live-autonomy/settings   -H 'content-type: application/json'   -d '{"actor":"tg_<YOUR_TELEGRAM_ID>","enabled":true,"daily_live_budget_usdt":"100","per_live_trade_max_usdt":"25","min_calibrated_conviction":"0.75"}'

curl -s 'http://127.0.0.1:8080/live-autonomy/settings?actor=tg_<YOUR_TELEGRAM_ID>'
curl -s 'http://127.0.0.1:8080/live-autonomy/today?actor=tg_<YOUR_TELEGRAM_ID>'
